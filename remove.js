...
const Pelicula = require("./pelicula"); // requerimos el modelo
...
async function eliminaActor(id_pelicula, actor, response) {
    ...
    let peliculaActualizada = await Pelicula.deleteOne({ _id: id_pelicula }, { $pull: { actors: 1, _id: 0 } }); //{ $pull: { actors: actor } }elimina actor que hemos colocada en postman
...
async function eliminaDirector(id_pelicula, director, response) {
    let peliculaActualizada = await Pelicula.deleteOne({ _id: id_pelicula }, { $pull: { directors: director } }); //{ $pull:  { writers: guionista }}elimina guionista que hemos colocada en postman
    find.devuelveUnaPelicula(id_pelicula, response);
